document.addEventListener('DOMContentLoaded', function () {
    ...
    loadScript('https://cdn.jsdelivr.net/npm/sortablejs@1.15.0/Sortable.min.js', function () {
        initInlineSortable();
        // El bulk sortable se inicia cuando se agregan imágenes
    });
    ...
    style.textContent = `
        ...
        .drag-handle { cursor:grab; font-size:1.3rem; color:#d2a8dd; padding:0 10px; user-select:none; }
        .drag-handle:active { cursor:grabbing; }
        .bulk-ghost { opacity:0.4; border:2px dashed #ffc1ea !important; }
        #bulk-preview-grid > div { cursor:grab !important; }
        #bulk-preview-grid > div:active { cursor:grabbing !important; transform:scale(1.05); }
        .bulk-orden-badge {
        ...
        .bulk-drag-hint { font-size:0.75rem; color:#a07ab0; margin-top:6px; display:block; }
    `;
    document.head.appendChild(style);

    // ── INLINE SORTABLE (filas de imágenes) ───────────────────
    function initInlineSortable() {
    ...
    function addDeleteButtons(tbody) {
        tbody.querySelectorAll('tr.form-row, tr.dynamic-imagenproducto_set').forEach(row => {
            ...
            btn.addEventListener('click', function () {
                ...
                const imgHtml = img ? `<img src="${img.src}" style="width:80px;height:80px;object-fit:cover;border-radius:8px;margin:10px auto;display:block;">` : '';
                Swal.fire({
                    title: '¿Eliminar imagen?',
                    html: `${imgHtml}<p style="color:#888;font-size:0.9rem;margin-top:8px;">Esta acción no se puede deshacer.</p>`,
                    icon: 'warning', showCancelButton: true,
                    confirmButtonText: 'Sí, eliminar', cancelButtonText: 'Cancelar',
                    confirmButtonColor: '#ffc1ea', cancelButtonColor: '#f0f0f0',
                    customClass: { confirmButton: 'swal-confirm-magie', cancelButton: 'swal-cancel-magie', popup: 'swal-popup-magie' }
                }).then(result => {
                    if (result.isConfirmed) {
                        deleteCheckbox.checked = true;
                        row.style.opacity = '0.4';
                        row.style.pointerEvents = 'none';
                        Swal.fire({ title: '¡Eliminada!', text: 'La imagen será eliminada al guardar.', icon: 'success', timer: 1800, showConfirmButton: false, customClass: { popup: 'swal-popup-magie' } });
                    }
                });
            });
    ...
    // ── BULK PREVIEW SORTABLE ─────────────────────────────────
    // Se llama desde change_form.html después de que todas las imágenes cargaron
    window.initBulkSortable = function () {
        const grid = document.getElementById('bulk-preview-grid');
        if (!grid || !window.Sortable) return;
        if (grid._sortableInstance) {
            grid._sortableInstance.destroy();
            grid._sortableInstance = null;
        }
    ...
    window.actualizarOrdenBulk = function () {
        const grid = document.getElementById('bulk-preview-grid');
        if (!grid) return;
        ...
        hidden.value = Array.from(grid.children).map(item => item.dataset.fileIndex || '0').join(',');
    };
});
